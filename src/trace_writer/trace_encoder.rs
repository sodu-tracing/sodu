...
#[inline(always)]
fn encode_trace(service_id: u64,span: &Span,mut buffer: &mut Vec<u8>){
     ...
     for attribute in attributes{
       ...
       // attribute_size = attribute_size + key_size as u16;
        ...
        match val{
            ...
            _ => {
                panic!("ub at encoding attributes")
            },
        }
    }
...
mod tests{
    use super::*;
    use test::Bencher;
    use crate::proto::common::{KeyValue, AnyValue, AnyValue_oneof_value};
    use protobuf::{SingularPtrField, RepeatedField, Message};
    use rand;
    use rand::Rng;
    fn generate_random_spans() -> Vec<Span>{
        ...
        val.value = Some(AnyValue_oneof_value::string_value(String::from("let's make it right")));
